...
class BuildFortificationValley
{
    ...
    {
        $r = $this->PgGame->query("INSERT INTO build_fortification_valley
...
VALUES ($1, $2, 'P', 1, $3, '-', now(), now(), $4, 0, now() + interval '$_build_time second')", [$_buil_pk, $_m_fort_pk, $_build_number, $_build_time]);
        ...
        $queue_pk = $this->PgGame->currSeq('build_fortification_valley_buil_fort_vall_pk_seq');
    ...
    {
        $this->PgGame->query('SELECT buil_pk, m_fort_pk, build_number, start_dt FROM build_fortification_valley WHERE buil_fort_vall_pk = $1', [$_queue_pk]);
        if (!$this->PgGame->fetch()) {
            return false;
        }
        $r = &$this->PgGame->row;

        $this->PgGame->query('UPDATE build_fortification_valley SET status = $2 WHERE buil_fort_vall_pk = $1', [$_queue_pk, 'F']);
    ...
    {
        $this->PgGame->query('SELECT buil_pk, m_fort_pk, build_number FROM build_fortification_valley WHERE buil_fort_vall_pk = $1', [$_queue_pk]);
        if (!$this->PgGame->fetch()) {
            return false;
        }
        $r = &$this->PgGame->row;

        $this->PgGame->query('UPDATE build_fortification_valley SET status = $2 WHERE buil_fort_vall_pk = $1', [$_queue_pk, 'C']);
        $this->PgGame->query('UPDATE build SET concurr_curr = concurr_curr - 1, last_update_dt = now() WHERE buil_pk = $1 AND concurr_curr > 0', [$r['buil_pk']]);
        $this->PgGame->query('UPDATE build SET status = $2 WHERE buil_pk = $1 AND concurr_curr = 0', [$r['buil_pk'], 'I']);
