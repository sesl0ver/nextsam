$(document).ready(function(){
	...
	var lord_hero_list = function ()
	{
		// GM 로그
		table_to_jqGrid('lord_own_hero', 'pager_lord_own_hero', '군주 보유 등용 대기 중 영웅 목록', ['No.', '영웅명', '레어도', '소켓', '강화', '상태', '충성'], // , '거래 여부'
		[
	...
	$('#hero_slot_info').dialog({
		...
		open: function() {
			...
			console.log(hero_slot_info);

			if (hero_slot_info['status'] == 'G') {
				$('#hero_manage_abadon').show().unbind().bind('mouseup', function(){
					var post_data = {};
					post_data['hero_pk'] = hero_slot_info['hero_pk'];

					$.post('/admin/gm/api/changeHeroStatus/abandon', post_data, function(data) {
						if (data.result == 'ok')
						{
				...
				$('#hero_manage_abadon').hide().unbind();
			}
		}
	});
});