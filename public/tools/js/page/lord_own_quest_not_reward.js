$(document).ready(function() {
	...
	// GM 로그
	table_to_jqGrid('lord_own_quest_not_reward', 'pager_lord_own_quest_not_reward', '보상 미완료 퀘스트', ['No.', 'm_ques_pk', '메인타이틀', '서브타이틀', '보상', '등록시간', '완료시간'], [
			...
			{'name' : 'last_up_dt', 'index' : 'last_up_dt', 'width' : 120, 'align' : 'center', 'sortable' : false},
		],
		false,
		function(_pk){
	...
	$('#quest_modify_form').dialog({
		...
		buttons: {
			"진행 중 퀘스트로 바꾸기" : function() {
				...
				var rthis = this;
				var post_data = {'lord_pk' : gm_info.selected_lord_pk, 'selected_my_quest_pk' : selected_my_quest_pk, 'now_state' : 'non_reward', 'change_state' : 'progress', 'cause' : cause};
				$.post('/admin/gm/api/changeQuestState', post_data, (data) => {
					if (data.result == 'fail')
					{
						alert(data.msg);
						return false;
			...
			"보상 완료 퀘스트로 바꾸기" : function() {
				...
				var rthis = this;
				var post_data = {'lord_pk' : gm_info.selected_lord_pk, 'selected_my_quest_pk' : selected_my_quest_pk, 'now_state' : 'non_reward', 'change_state' : 'rewarded', 'cause' : cause};
				$.post('/admin/gm/api/changeQuestState', post_data, (data) => {
					if (data.result == 'fail')
					{
						alert(data.msg);
