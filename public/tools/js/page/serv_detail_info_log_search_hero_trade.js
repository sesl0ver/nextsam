$(document).ready(function(){
	...
	// 쿼리 스트링으로 lord_name을 받았다면 곧장 군주명으로 큐빅 차감, 충전 내역을 보도록 한다.
	var query_str = String(document.location.search).substr(1);

	var _get = {};

	query_str.replace(/([^=]+)=([^&]*)(&|$)/gi, function(origin, key, value){
		_get[key] = decodeURIComponent(value);
		return origin;
	});
	if (typeof _get['lord_name'] == 'string')
	{
		$('#lord_name').val(_get['lord_name']);
		$('#search_type_desc_cash').attr('checked', true).change();
		$('#search_type_incr_cash').attr('checked', true).change();
		$('#search_log').mouseup();
	}
...
jqGrid_templete = {
	'etc' : {
		'header' : ['일시', '거래번호', '구매번호', '구분', '좌표', '실행군주', '판매군주', '거래/입찰가', '최소/최대입찰', '상세'],
		'body' : [
			{'name' : 'log_date', 'index' : 'log_date', 'width' : 100, 'align' : 'center', 'sortable' : false},
			{'name' : 'hero_trad_pk', 'index' : 'hero_trad_pk', 'width' : 100, 'align' : 'center', 'sortable' : false},
			{'name' : 'hero_trad_bid_pk', 'index' : 'hero_trad_bid_pk', 'width' : 100, 'align' : 'center', 'sortable' : false},
			{'name' : 'type', 'index' : 'type', 'align' : 'center', 'sortable' : false},
			{'name' : 'posi_pk', 'index' : 'posi_pk', 'width' : 100, 'align' : 'center', 'sortable' : false},
			{'name' : 'sale_lord_name', 'index' : 'sale_lord_name', 'width' : 120, 'align' : 'center', 'sortable' : false},
			{'name' : 'lord_name', 'index' : 'lord_name', 'width' : 120, 'align' : 'center', 'sortable' : false},
			{'name' : 'gold', 'index' : 'gold', 'width' : 100, 'align' : 'center', 'sortable' : false},
			{'name' : 'minmax', 'index' : 'minmax', 'width' : 100, 'align' : 'center', 'sortable' : false},
			{'name' : 'description', 'index' : 'description', 'width' : 400, 'align' : 'left', 'sortable' : false}
		]
	}
}
...
function get_log_search_result(appendTarget, post_data)
{
	...
	var templete = jqGrid_templete['etc'];
