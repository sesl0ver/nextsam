$(document).ready(function(){
	...
	$( "#dialog-form" ).dialog({
		...
		buttons: {
			"채팅 차단하기": () => {
				...
				{
					alert('사유를 적지 않으면 유저의 채팅을 차단할 수 없습니다.');
					return false;
				}
				...
				$.post('/admin/gm/api/chatBlock', do_post_data, (data) => {
					if (data.result === 'ok') {
						userkick(post_lord_pk);
						alert('선택한 유저의 채팅은 차단하였습니다.');
						$(this).dialog("close");
	...
	function get_lord_search_by_account_result(appendTarget, post_data)
	{
		...
		table_to_jqGrid(
			...
			pagerId,
			'검색 결과 ',
			['No.', '군주명', '계정 등록일', '마지막 로그인 시간', '차단 여부', '차단 시작', '차단 해제', '로그인 여부'],
			[
			...
			function(id){
				...
				if (confirm("선택한 유저의 채팅을 차단 하시겠습니까?")) {
					post_lord_pk = id;
