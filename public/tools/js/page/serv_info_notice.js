...
$(document).ready(function() {
	...
	function deleteNotice (_pk)
	{
		...
		$.post('/admin/gm/api/deleteNotice', { pk: _pk }, function(data, status) {
			getNoticeList();
			clearForm();
		}, 'json');
	}
	...
	{
		$.post('/admin/gm/api/modifyNotice', { pk: _pk }, function(data, status) {
			let row = data.row;
			...
			notice_start_datetime.val(moment(row.start_dt).tz('Etc/UTC').format('YYYY-MM-DD HH:mm'));
			notice_end_datetime.val(moment(row.end_dt).tz('Etc/UTC').format('YYYY-MM-DD HH:mm'));
			modify_pk = _pk;
	...
	update_button.mouseup(function() {
		...
		$.post('/admin/gm/api/updateNotice', {
			pk: modify_pk,
			target_server: target,
			start_time: start_time,
			end_time: end_time,
	...
	function getNoticeList ()
	{
		...
		$.post('/admin/gm/api/noticeList', {}, function(data, status) {
			...
			for (let notice of Object.values(data.list)) {
				...
				tr.append($('<td></td>').html(moment(notice.start_dt).tz('Etc/UTC').format('YYYY-MM-DD HH:mm') + '<br />~<br />' + moment(notice.end_dt).tz('Etc/UTC').format('YYYY-MM-DD HH:mm')));
				tr.append($('<td></td>').text(notice.repeat === 't' ? `${notice.repeat_time}분` : '없음'));
				tr.append($('<td></td>').text(notice.active === 't' ? '활성' : '비활성'));
				let next_dt = '종료됨';
				if (notice.used !== 't') {
					next_dt = moment(notice.next_dt).tz('Etc/UTC').format('YYYY-MM-DD HH:mm');
				}
				tr.append($('<td></td>').text(next_dt));
				tr.append($('<td></td>').text(moment(notice.regist_dt).tz('Etc/UTC').format('YYYY-MM-DD HH:mm')));
