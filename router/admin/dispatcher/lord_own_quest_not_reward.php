...
$app->post('/admin/gm/api/lord_own_quest_not_reward', $Render->wrap(function (array $params) use ($Render, $i18n) {
    ...
    $PgGame->query("SELECT count(my_ques_pk) FROM my_quest where status = 'C' AND lord_pk = $1  AND reward_status = 'N'", [$_SESSION['selected_lord']['lord_pk']]);
    ...
    $PgGame->query("SELECT my_ques_pk, m_ques_pk, date_part('epoch', start_dt)::integer as start_dt, date_part('epoch', last_up_dt)::integer as last_up_dt FROM my_quest where status = 'C' AND lord_pk = $1 AND reward_status = 'N' ORDER BY m_ques_pk LIMIT $2 OFFSET $3", [$_SESSION['selected_lord']['lord_pk'], $limit, $offset_start]);
    $PgGame->fetchAll();
    $rows = $PgGame->rows;
