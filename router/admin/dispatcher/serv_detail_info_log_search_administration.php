...
$app->post('/admin/gm/api/serv_detail_info_log_search_administration', $Render->wrap(function (array $params) use ($Render, $i18n) {
    ...
    $PgLog->query("SELECT log_date, web_id, lord_pk, posi_pk, type, description FROM log_building_administration{$where} order by log_date desc limit $limit offset {$offset}", $query_params);
    $PgLog->fetchAll();
    ...
    foreach ($PgLog->rows as $row) {
        ...
        $response[] = [$row['log_date'], $row['web_id'], $_lord_name[$row['lord_pk']], $row['posi_pk'], $row['type'], $row['description']];
    }
