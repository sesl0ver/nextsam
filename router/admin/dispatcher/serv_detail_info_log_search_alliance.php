...
$app->post('/admin/gm/api/serv_detail_info_log_search_alliance', $Render->wrap(function (array $params) use ($Render, $i18n) {
    ...
    $PgLog->query("SELECT log_date, web_id, lord_pk, posi_pk, type, description FROM log_building_alliance{$where} order by log_date desc limit $limit offset {$offset}", $query_params);
    $PgLog->fetchAll();
