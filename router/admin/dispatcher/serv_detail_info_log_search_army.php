...
$app->post('/admin/gm/api/serv_detail_info_log_search_army', $Render->wrap(function (array $params) use ($Render, $i18n) {
    ...
    $PgLog->query("SELECT log_date, web_id, lord_pk, posi_pk, type, m_army_pk, army_info, web_channel, server_index, build_number FROM log_army{$where} order by log_date desc limit $limit offset {$offset}", $query_params);
    $PgLog->fetchAll();
    ...
    foreach ($PgLog->rows as $row) {
        ...
        $response[] = [$row['log_date'], $row['web_id'], $_lord_name[$row['lord_pk']], $row['posi_pk'], $row['type'], $row['m_army_pk'], $row['build_number'], $row['type'].'|'.$row['army_info']];
    }
    return $Render->nsXhrReturn('success', null, $response);
    // [date('Y-m-d H:i:s', $PgLog->row['log_date']), $PgLog->row['lord_pk'], $PgLog->row['posi_pk'], getLogArmyType($PgLog->row['type']), $PgLog->row['build_number'], getArmyInfo($PgLog->row['army_info'], $PgLog->row['type'])]
}));
