...
$app->post('/admin/gm/api/serv_detail_info_log_search_combi', $Render->wrap(function (array $params) use ($Render, $i18n) {
    ...
    if (isset($params['search_type'])) {
        $where .= (($where !== '') ? ' AND ' : ' WHERE ');
        $where .= "combi_type IN ('" . str_replace(',', "','", $params['search_type']) . "')";
    }

    $limit = 500;
    $offset = ($params['list_offset'] - 1) * $limit;

    $PgLog->query("SELECT log_date, lord_pk, posi_pk, hero_pk, m_hero_pk, combi_type, serial_num, used_type, consume_gold FROM log_hero_combi{$where} order by log_date desc limit $limit offset {$offset}", $query_params);
    $PgLog->fetchAll();
    ...
    foreach ($PgLog->rows as $row) {
        ...
        $response[] = [$row['log_date'], $_lord_name[$row['lord_pk']], $row['posi_pk'], $row['combi_type'], $row['hero_pk'], $row['m_hero_pk'], $row['used_type'], $row['consume_gold'], $row['serial_num']];
    }
