...
$app->post('/admin/gm/api/serv_detail_info_log_search_encounter', $Render->wrap(function (array $params) use ($Render, $i18n) {
    ...
    $PgLog->query("SELECT log_date, web_id, lord_pk, posi_pk, type, m_hero_pk, distance_type, m_item_pk, encounter_type, encounter_value, invitation_cnt, yn_invited FROM log_hero_encounter{$where} order by log_date desc limit $limit offset {$offset}", $query_params);
    $PgLog->fetchAll();
    ...
    foreach ($PgLog->rows as $row) {
        ...
        $response[] = [$row['log_date'], $row['web_id'], $_lord_name[$row['lord_pk']], $row['posi_pk'], $row['type'], $row['m_hero_pk'], $row['distance_type'], $row['m_item_pk'], $row['encounter_type'].':'.$row['encounter_value'], $row['invitation_cnt'], $row['yn_invited']];
    }
