...
$app->post('/admin/gm/api/serv_detail_info_log_search_hero_enchant', $Render->wrap(function (array $params) use ($Render, $i18n) {
    ...
    $PgLog->query("SELECT log_date, web_id, lord_pk, posi_pk, type, hero_pk, m_hero_pk, m_item_pk, enchant_cnt, description FROM log_hero_enchant{$where} order by log_date desc limit $limit offset {$offset}", $query_params);
    $PgLog->fetchAll();
    ...
    foreach ($PgLog->rows as $row) {
        ...
        $response[] = [$row['log_date'], $row['web_id'], $_lord_name[$row['lord_pk']], $row['posi_pk'], $row['hero_pk'], $row['m_hero_pk'], $row['m_item_pk'], $row['enchant_cnt'], $row['type'], $row['description']];
    }
    return $Render->nsXhrReturn('success', null, $response);
    // [date('Y-m-d H:i:s', $PgLog->row['log_date']), $PgLog->row['lord_pk'], $PgLog->row['posi_pk'], getLogEnchType($PgLog->row['type']), $PgLog->row['m_hero_pk'], $PgLog->row['m_item_pk'], $PgLog->row['enchant_cnt'], getDescription($PgLog->row['type'], $PgLog->row['description'])];
