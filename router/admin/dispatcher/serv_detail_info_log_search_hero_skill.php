...
$app->post('/admin/gm/api/serv_detail_info_log_search_hero_skill', $Render->wrap(function (array $params) use ($Render, $i18n) {
    ...
    $PgLog->query("SELECT log_date, lord_pk, posi_pk, hero_pk, m_hero_pk, type, m_hero_skil_pk, m_item_pk, slot_pk, description FROM log_hero_skill{$where} order by log_date desc limit $limit offset {$offset}", $query_params);
    $PgLog->fetchAll();
    ...
    foreach ($PgLog->rows as $row) {
        ...
        $response[] = [$row['log_date'], $_lord_name[$row['lord_pk']], $row['posi_pk'], $row['hero_pk'], $row['m_hero_pk'], $row['type'], $row['m_hero_skil_pk'], $row['m_item_pk'], $row['slot_pk'], $row['description']];
    }
