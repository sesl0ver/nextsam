...
$app->post('/admin/gm/api/serv_detail_info_log_search_hero_skill_active', $Render->wrap(function (array $params) use ($Render, $i18n) {
    ...
    $PgLog->query("SELECT log_date, lord_pk, posi_pk, m_hero_pk, type, rand_value, skil_value, description FROM log_hero_skill_active{$where} order by log_date desc limit $limit offset {$offset}", $query_params);
    $PgLog->fetchAll();
    ...
    foreach ($PgLog->rows as $row) {
        ...
        $response[] = [$row['log_date'], $_lord_name[$row['lord_pk']], $row['posi_pk'], $row['m_hero_pk'], $row['type'], $row['rand_value'], $row['skil_value'], $row['description']];
    }
