...
$app->post('/admin/gm/api/serv_detail_info_log_search_invitation', $Render->wrap(function (array $params) use ($Render, $i18n) {
    ...
    $limit = 500;
    $offset = ($params['list_offset'] - 1) * 500;

    $PgLog->query("SELECT log_date, web_id, lord_pk, posi_pk, type, m_hero_pk, send_gold, result_value, result_status, invit_cnt, enco_hero_pk FROM log_hero_invitation{$where} order by log_date desc limit $limit offset {$offset}", $query_params);
    $PgLog->fetchAll();
    ...
    foreach ($PgLog->rows as $row) {
        ...
        $response[] = [$row['log_date'], $row['web_id'], $_lord_name[$row['lord_pk']], $row['posi_pk'], $row['type'], $row['m_hero_pk'], $row['send_gold'], $row['result_status'], $row['result_value'], $row['invit_cnt'], $row['enco_hero_pk']];
    }
