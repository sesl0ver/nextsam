...
$app->post('/admin/gm/api/serv_detail_info_log_search_item', $Render->wrap(function (array $params) use ($Render, $i18n) {
    ...
    $PgLog->query("SELECT log_date, lord_pk, posi_pk, type, acquire_type, m_item_pk, price, quantity, description, web_channel, server_index FROM log_item{$where} order by log_date desc limit $limit offset {$offset}", $query_params);
    $PgLog->fetchAll();
    ...
    foreach ($PgLog->rows as $row) {
        ...
        $response[] = [$row['log_date'], $_lord_name[$row['lord_pk']], $row['posi_pk'], $row['type'], $row['acquire_type'], $row['m_item_pk'], $row['quantity'], $row['description']];
    }
