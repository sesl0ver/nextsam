...
$app->post('/admin/gm/api/serv_detail_info_log_search_resource', $Render->wrap(function (array $params) use ($Render, $i18n) {
    ...
    if (isset($params['search_type'])) {
        $where .= (($where !== '') ? ' AND ' : ' WHERE ');
        $where .= "type SIMILAR TO '%(" . str_replace(',', "|", $params['search_type']) . ")%'";
    }

    $limit = 500;
    $offset = ($params['list_offset'] - 1) * $limit;

    $PgLog->query("SELECT log_date, lord_pk, posi_pk, type, amount, web_channel, server_index FROM log_resource{$where} order by log_date desc limit $limit offset {$offset}", $query_params);
    $PgLog->fetchAll();
    ...
    foreach ($PgLog->rows as $row) {
        ...
        $response[] = [$row['log_date'], $_lord_name[$row['lord_pk']], $row['posi_pk'], $row['type'], $row['amount']];
    }
