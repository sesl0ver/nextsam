...
$app->post('/admin/gm/api/serv_detail_info_log_search_suppress', $Render->wrap(function (array $params) use ($Render, $i18n) {
    ...
    $PgLog->query("SELECT log_date, web_id, lord_pk, type, posi_pk, target_level, suppress_cnt, target_cnt, target_posi_pk_1, target_posi_pk_2, target_posi_pk_3, army_type_1, army_type_2, army_type_3 FROM log_suppress{$where} order by log_date desc limit $limit offset {$offset}", $query_params);
    $PgLog->fetchAll();
    ...
    foreach ($PgLog->rows as $row) {
        ...
        $response[] = [$row['log_date'], $row['web_id'], $_lord_name[$row['lord_pk']], $row['posi_pk'], $row['type'], $row['target_level'], $row['target_posi_pk_1'].'|'.$row['target_posi_pk_2'].'|'.$row['target_posi_pk_3'], $row['target_cnt'], $row['suppress_cnt']];
    }
    return $Render->nsXhrReturn('success', null, $response);
    // [date('Y-m-d H:i:s', $PgLog->row['log_date']), $PgLog->row['web_id'], $PgLog->row['lord_pk'], $PgLog->row['posi_pk'], getLogSuppType($PgLog->row['type']), $PgLog->row['target_level'], getSuppInfo($info_array[$i])];
}));
