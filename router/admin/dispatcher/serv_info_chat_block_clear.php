...
$app->post('/admin/gm/api/serv_info_chat_block_clear', $Render->wrap(function (array $params) use ($Render, $i18n) {
...
$app->post('/admin/gm/api/chatBlockClear', $Render->wrap(function (array $params) use ($Render, $i18n) {
    if (! isset($_SESSION) || ! isset($_SESSION['gm_active'])) {
        // 세션이 설정되지 않은 경우 이 파일을 볼 수없음.
        header("HTTP/1.0 404 Not Found");
        return $Render->view();
    ...
    if ($block_info['is_chat_blocked'] == 'N') {
    ...
    $result = $PgGame->query('UPDATE lord SET is_chat_blocked = $1, chat_block_start_dt = NULL, chat_block_end_dt = NULL WHERE lord_pk = $2', ['N', $params['lord_pk']]);
    if (! $result) {
